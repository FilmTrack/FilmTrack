        ...
        <div className="flex min-h-16 items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3 lg:gap-5">
            ...
            <nav aria-label="ناوبری اصلی" className="hidden items-center gap-0.5 md:flex">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className={desktopLinkClass}><Tv className="h-4 w-4" /> سریال‌ها <ChevronDown className="h-3.5 w-3.5" /></DropdownMenuTrigger>
                <DropdownMenuContent className="border-white/10 bg-[#0b1220]/98 text-white shadow-2xl backdrop-blur-xl">
                  <Link href="/shows" className="block cursor-pointer"><DropdownMenuItem>سریال‌های محبوب</DropdownMenuItem></Link>
                </DropdownMenuContent>
              </DropdownMenu>

              <DropdownMenu>
                <DropdownMenuTrigger className={desktopLinkClass}><Film className="h-4 w-4" /> فیلم‌ها <ChevronDown className="h-3.5 w-3.5" /></DropdownMenuTrigger>
                <DropdownMenuContent className="border-white/10 bg-[#0b1220]/98 text-white shadow-2xl backdrop-blur-xl">
                  <Link href="/movies" className="block cursor-pointer"><DropdownMenuItem>فیلم‌های محبوب</DropdownMenuItem></Link>
                </DropdownMenuContent>
              </DropdownMenu>

              <DropdownMenu>
                <DropdownMenuTrigger className={desktopLinkClass}><ListVideo className="h-4 w-4" /> ژانرها <ChevronDown className="h-3.5 w-3.5" /></DropdownMenuTrigger>
              ...
              <Link href="/discover" className={`${desktopLinkClass} text-violet-300 hover:text-violet-200`}><Compass className="h-4 w-4" /> کشف فارسی</Link>
              <Link href="/calendar" className={desktopLinkClass}><Calendar className="h-4 w-4" /> تقویم</Link>

              {isLoggedIn ? (
                <DropdownMenu>
                  <DropdownMenuTrigger className={`${desktopLinkClass} text-violet-300 hover:text-violet-200`}><Sparkles className="h-4 w-4" /> برای من <ChevronDown className="h-3.5 w-3.5" /></DropdownMenuTrigger>
                  <DropdownMenuContent className="w-56 border-white/10 bg-[#0b1220]/98 text-white shadow-2xl backdrop-blur-xl">
                    ...
                    <Link href="/dashboard/notifications" className="block cursor-pointer"><DropdownMenuItem><Bell className="ml-2 h-4 w-4" /> اعلان‌های من</DropdownMenuItem></Link>
                    ...
                    <Link href="/dashboard/account" className="block cursor-pointer"><DropdownMenuItem><Settings className="ml-2 h-4 w-4" /> حساب و حریم خصوصی</DropdownMenuItem></Link>
                  </DropdownMenuContent>
                </DropdownMenu>
              ) : null}

              <Link href="/plus" className={`${desktopLinkClass} text-blue-300 hover:text-blue-200`}><Sparkles className="h-4 w-4" /> پلاس</Link>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isLoggedIn ? (
              <>
                <Link href="/dashboard/today" className="hidden min-h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 text-sm font-bold text-slate-300 transition hover:bg-white/[0.07] hover:text-white sm:inline-flex"><UserRound className="h-4 w-4" /> امروز من</Link>
                <LogoutButton />
              </>
            ) : (
              ...
              <nav className="fixed inset-x-3 top-[72px] z-50 max-h-[calc(100vh-88px)] overflow-y-auto rounded-2xl border border-white/10 bg-[#09111f]/98 p-2 text-sm text-slate-200 shadow-2xl backdrop-blur-xl sm:left-4 sm:right-auto sm:min-w-72">
                ...
                {isLoggedIn ? (
                  <>
                    ...
                    <Link href="/dashboard/notifications" className="flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 font-black text-amber-200 hover:bg-amber-500/10"><Bell className="h-4 w-4" /> اعلان‌های من</Link>
                    ...
                    <Link href="/dashboard/account" className="flex min-h-12 items-center gap-3 rounded-xl px-3 py-2 font-bold hover:bg-white/[0.06]"><Settings className="h-4 w-4 text-slate-300" /> حساب و حریم خصوصی</Link>
                  </>
                ) : null}
                ...
                {isLoggedIn && <Link href="/dashboard" className="mt-1 flex min-h-12 items-center gap-3 border-t border-white/10 px-3 py-2 font-bold hover:bg-white/[0.06]"><UserRound className="h-4 w-4" /> پنل من</Link>}
              </nav>
            </details>
          </div>
        </div>
